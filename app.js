...
function evaluateQuestion(q, mode, userRating, userRanking) {
  const targetRanking = q.rankingArray; // e.g. ['B', 'C', 'A', 'D']

  if (mode === 'B') {
    // Pairwise comparison (6 pairs)
    let pairsCorrect = 0;
    const totalPairs = 6;
    const pairAnalysis = [];

    for (let i = 0; i < targetRanking.length; i++) {
      for (let j = i + 1; j < targetRanking.length; j++) {
        const item1 = targetRanking[i];
        const item2 = targetRanking[j];
        const userPos1 = userRanking.indexOf(item1);
        const userPos2 = userRanking.indexOf(item2);
        const isCorrect = userPos1 < userPos2;
    ...
    const percentage = Math.round((pairsCorrect / totalPairs) * 100);
    return {
      mode: 'B',
      score: pairsCorrect,
      maxScore: totalPairs,
    ...
    ['A', 'B', 'C', 'D'].forEach(letter => {
      const uVal = userRating[letter] !== null ? userRating[letter] : 4;
      const tVal = targetScores[letter];
      const diff = Math.abs(uVal - tVal);
      const itemScore = Math.max(0, 1.5 - (diff * 0.25));
